...
function initApp() {
  products.forEach((value, key) => {
    ...
    newDiv.innerHTML = `
        <div class="prods">
        <img src="images/${value.image}" class="prodimg">
        <div class="title">${value.name}</div>
        <div class="price">${value.price.toLocaleString()}</div>
        <button class="prodbuttons" onclick="addToCard(${key})">Add To Cart</button>
    </div>`;
...
function reloadCard() {
  ...
  listCards.forEach((value, key) => {
    ...
    if (value != null) {
      ...
      newDiv.innerHTML = `
                <div0 class = "cartimg"><img src="images/${value.image}"/></div>
                <div>${value.name}</div>
